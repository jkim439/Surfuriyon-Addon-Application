...
--	$Rev: 3377 $
--	$Date: 2006-04-06 19:20:23 -0500 (Thu, 06 Apr 2006) $
--]]

--Compatible with the following Mini-Libs
Sea.versions.SeaString = 0.10;	-- SeaString
...
Sea.string = {
	...
	fromTime = function (time, decimalplaces)
		...
		local seconds = mod(floor(time*size), 60*size);
		...
		if (hours > 0) then
			return string.format("%d:%.2d:%s%f", hours, minutes, spfx, seconds);
		else
			return string.format("%d:%s%f", minutes, spfx, seconds);
		end
	end; 
