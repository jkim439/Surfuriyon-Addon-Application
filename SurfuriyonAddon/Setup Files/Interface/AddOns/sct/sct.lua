...
function SCT_Initialize()
	...
	SLASH_SCTMENU1 = "/전투메세지설정";
	
	SlashCmdList["SCTRESET"] = SCT_Reset;
	SLASH_SCTRESET1 = "/전투메세지초기화";
	
	SlashCmdList["SCTDISPLAY"] = SCT_CmdDisplay;
	SLASH_SCTDISPLAY1 = "/전투메세지표시";
   ...
   if ( SCT_CONFIG == nil) then
	...
	--set player name
	SCT_PlayerName = GetCVar("realmName").." 서버의 "..UnitName("player");
...
function SCT_ParseCombat(event, arg1)
	...
	elseif (event == "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE" or
		...
		if target and string.find(target, "(.+)%|1이;가;") then
			return;
		end
		if damage then
			SCT_Display_Only("SHOWSPELL", "-"..damage, nil, damagetype, SCT_FindResists());
		...
		if spell and string.find(spell, "(.+)%|1이;가;") then
			return;
		end
		if damage then
			SCT_Display_Only("SHOWSPELL", "-"..damage, nil, damagetype, SCT_FindResists());
		...
		if debuff and string.find(debuff, "(.+)%|1이;가;") then
			return;
		end
		if debuff then
	...
	elseif (event == "CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS") then
		...
		if damage then
			if string.find(arg1, "(.+)의 (.+)의 (.+)%|1으로;로;") then
				target = string.gsub(target, "(.+)의 (.+)", "%1");
			end
	...
	elseif (event == "CHAT_MSG_SPELL_HOSTILEPLAYER_BUFF") then
		...
		if target then
			if string.find(arg1, "(.+)의 (.+)의 (.+)%|1이;가;") then
				target = string.gsub(target, "(.+)의 (.+)", "%1");
			end
			...
			for carg1, carg2, carg3, carg4, carg5 in string.gfind(arg1, value.search) do
				...
				if ((value.argcount) and (value.argcount > 0) and (value.argcount < 6)) then
					if ( value.argcount == 3 ) then
						local _,_,prefix,suffix = string.find(carg1, "(.+)의 (.+)");
						if ( prefix and suffix ) and ( string.find(prefix, "(.+)의 (.+)") or string.find(suffix, "(.+) (.+)") or suffix == "영웅심" or suffix == "재생" ) then
							return;
						end
					end
