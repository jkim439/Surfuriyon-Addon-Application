...
function aftt_buttonToggle(option)
	...
	elseif (option == "quest") then
		if (AF_ToolTip[aftt_localUser]["Quest"] == 1) then
			AF_ToolTip[aftt_localUser]["Quest"] = 0;
		else
			AF_ToolTip[aftt_localUser]["Quest"] = 1;
		end
...
local mouseover_targetname = nil;
function aftt_onRefresh()
--[[ START : 메모리 증가 수정 : 이름이 같으면 갱신하지 않게 함. 아래쪽의 다음과 같은 코드가 재귀되므로 발생하는 문제임.
	GameTooltipTextLeft2:SetText("|cff" .. aftt_color_description .. GameTooltipTextLeft2:GetText() .. "|r")
]]
	if (mouseover_targetname and mouseover_targetname == UnitName("mouseover")) then
		return;
	end
	mouseover_targetname = UnitName("mouseover");
-- END: 메모리 증가 수정

	--Set our options frame refresh
		if (aftt_optionFrame:IsVisible()) then
			aftt_TextLeft1:SetText("위치:");
			aftt_TextRight1:SetText(AF_ToolTip[aftt_localUser]["Anchor"]);
			aftt_TextLeft2:SetText("좌표 가로:");
			aftt_TextRight2:SetText(AF_ToolTip[aftt_localUser]["PositionX"]);
			aftt_TextLeft3:SetText("좌표 세로:");
			...
			if (AF_ToolTip[aftt_localUser]["Guild"] == 1) then
				...
				aftt_TextRight8:SetText("보이기");
			elseif (AF_ToolTip[aftt_localUser]["Guild"] == 2) then
				aftt_TextLeft8:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight8:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight8:SetText("하단으로 보이기");
			else
				aftt_TextLeft8:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRight8:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRight8:SetText("감추기");
			end

			aftt_TextLeft9:SetText("계급:");
			if (AF_ToolTip[aftt_localUser]["Rank"] == 1) then
				aftt_TextLeft9:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight9:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight9:SetText("텍스트");
			elseif (AF_ToolTip[aftt_localUser]["Rank"] == 2) then
				aftt_TextLeft9:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight9:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight9:SetText("등급");
			else
				aftt_TextLeft9:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRight9:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRight9:SetText("감추기");
			end

			aftt_TextLeft10:SetText("퀘스트:");
			if (AF_ToolTip[aftt_localUser]["Quest"] == 1) then
				aftt_TextLeft10:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight10:SetVertexColor(1.0, 0.8, 0.0);
				aftt_TextRight10:SetText("보이기");
			else
				aftt_TextLeft10:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRight10:SetVertexColor(0.4, 0.4, 0.0);
				aftt_TextRight10:SetText("감추기");
			end

				aftt_TextLeft11:SetText(aftt_color_chart_name);
				aftt_TextLeft12:SetText(aftt_color_chart_author);
...
function aftt_onLoad()
	...
	--Set our frame with the name and version
		aftt_TextHeader:SetText("AF_" .. name .. " v" .. version .. "한글판");
		aftt_TextHeader2:SetText("현재 설정된 색상값");
		aftt_TextHeader:SetVertexColor(1.0, 1.0, 1.0);
		aftt_TextHeader2:SetVertexColor(1.0, 1.0, 1.0);
		aftt_TextLeft9:SetVertexColor(1.0, 0.8, 0.0);
		aftt_TextLeft10:SetVertexColor(1.0, 0.8, 0.0);
		aftt_TextLeft11:SetVertexColor(0.0, 0.6, 1.0);
	--Create update table
		aftt_updateFlag = {
			["Previous"] = "none";
			["Current"] = "none";
...
--					aftt_rankName = ("[" .. aftt_rankName .. "] ");
				...
				if (aftt_rankNumber == nil or aftt_rankNumber < 1) then
					...
					aftt_rankNumber = ("[등급 " .. aftt_rankNumber .. "] ");				
		...
		if (AF_ToolTip[aftt_localUser]["PvP"] ~= 1) then
			...
			if (UnitIsPVP(unit)) then
				if (aftt_updateFlag["PvP"] == 0) then
					GameTooltip:AddLine("|cffFFFFFF전쟁참여|r");
	...
	-----------------------------------------------------------
		if (AF_ToolTip[aftt_localUser]["Quest"] ~= 1) then
			...
			if( UnitInParty(unit) )then
				...
				if (aftt_updateFlag["QuestFlag"] == 0) then
					for i=1,numEntries,1 do
						questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete = GetQuestLogTitle(i);
						if ( isHeader ) then
						else
							isOnQuest = IsUnitOnQuest(i, unit);
							if ( isOnQuest and isOnQuest == 1 ) then
								GameTooltip:AddLine(LIGHTYELLOW_FONT_COLOR_CODE .. "[" .. level .. "] "..questLogTitleText .. FONT_COLOR_CODE_CLOSE);
							end
						end
					end
					aftt_updateFlag["QuestFlag"] = 1;
				end
			end
		end
