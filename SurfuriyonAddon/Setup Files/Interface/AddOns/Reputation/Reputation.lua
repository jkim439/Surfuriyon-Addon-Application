			...
			if (Reputations_Enabled==1) then
				if (Reputations[name] and Reputations_Notify==1) then
						...
						if (difference > 0) then --Reputation went up
							...
							if (standingID ~= 8) then
								...
								if (Reputations_Next==1) then
									...
									REPUTATIONS_CHAT_FRAME:AddMessage(format(getglobal("REPUTATION_NEEDED"..Reputations_Raw),name,getglobal("FACTION_STANDING_LABEL"..standingID+1),RepRemains), 1.0, 1.0, 0.0);
								end
								if (Reputations_Repeat==1) then
									...
									REPUTATIONS_CHAT_FRAME:AddMessage(format(getglobal("REPUTATION_REPEATS"),name,getglobal("FACTION_STANDING_LABEL"..standingID+1),RepRepeats), 1.0, 1.0, 0.0);
								end
							end
						elseif (difference < 0) then --Reputation went down
							...
							if (standingID ~= 1) then
								...
								if (Reputations_Next==1) then
									...
									REPUTATIONS_CHAT_FRAME:AddMessage(format(getglobal("REPUTATION_LEFT"..Reputations_Raw),name,getglobal("FACTION_STANDING_LABEL"..standingID-1),RepRemains), 1.0, 1.0, 0.0);
								end
								if (Reputations_Repeat==1) then
									RepRepeats = RepRemains/difference;
									if (RepRepeats > floor(RepRepeats)) then
					...
					elseif (Reputations[name].standingID < standingID) then --Reputation went up
						...
						REPUTATIONS_CHAT_FRAME:AddMessage(format(REPUTATION_REACHED,name,getglobal("FACTION_STANDING_LABEL"..standingID)), 1.0, 1.0, 0.0);
						if (standingID ~= 8) then
							if (Reputations_Next==1) then
								REPUTATIONS_CHAT_FRAME:AddMessage(format(getglobal("REPUTATION_NEEDED"..Reputations_Raw),name,getglobal("FACTION_STANDING_LABEL"..standingID+1),RepRemains), 1.0, 1.0, 0.0);
							end
