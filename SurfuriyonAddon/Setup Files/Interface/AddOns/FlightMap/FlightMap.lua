...
local function lSetDefaultData()
    ...
    local hordeValor = "1:461:226";
    local allianceValor = "1:463:223";
    for k, v in FlightMap[FLIGHTMAP_HORDE] or {} do
        if v.Costs then v.Costs[hordeValor] = nil; end
        if v.Flights then v.Flights[hordeValor] = nil; end
    end
    for k, v in FlightMap[FLIGHTMAP_ALLIANCE] or {} do
        if v.Costs then v.Costs[allianceValor] = nil; end
        if v.Flights then v.Flights[allianceValor] = nil; end
    end
    for k, v in FlightMap.Knowledge do
        v[hordeValor] = nil;
        v[allianceValor] = nil;
    end
    if FlightMap[FLIGHTMAP_HORDE] then
        FlightMap[FLIGHTMAP_HORDE][hordeValor] = nil;
    end
    if FlightMap[FLIGHTMAP_ALLIANCE] then
        FlightMap[FLIGHTMAP_ALLIANCE][allianceValor] = nil;
    end
...
function FlightMapOptionsFrame_OnShow()
    ...
    for optid, option in pairs(FLIGHTMAP_OPTIONS) do
        ...
        OptionsFrame_EnableCheckBox(button, FlightMap.Opts[option.option]);
        ...
        for _, child in option.children or {} do
            ...
            if other then
                if FlightMap.Opts[option.option] then
                    OptionsFrame_EnableCheckBox(other,
