...
function TitanPanelRepairButton_OnEvent()
    ...
    if ((event == "BAG_UPDATE") and (GetTime() - LastBagUpdate > 0.1)) and (TitanGetVar(TITAN_REPAIR_ID,"ShowInventory") == 1) then
...
function TitanRepair_GetInventoryInformation()
...
function TitanRepair_GetEquipedInformation()
	...
	for index, value in INVENTORY_ALERT_STATUS_SLOTS do
        ...
        if (min_index == 0) and (TitanGetVar(TITAN_REPAIR_ID,"ShowInventory") == 1) then
		TitanRepair_GetInventoryInformation();
	else
